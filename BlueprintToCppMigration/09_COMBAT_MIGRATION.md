...
Current validation is deliberately bounded: `QATS.QCombat.*` passes `23/23`, the complete `/Game` Blueprint sweep passes `4706/4706` with no compile warning, error or load failure, and a Standalone `L_Dev_Rz` run proved authoritative non-lethal damage, reset, lethal server kill and revive with a clean PIE Message Log. The later melee correction also passes an Editor compile and clean compilation of the character plus both animation Blueprints. Fresh player-visible melee validation, listen-server, dedicated-server and packaged Development/Shipping parity remain open and are not implied by this checkpoint.
...
The nine live player melee montages (punches plus every equipped melee animation set) now share one normalized native damage window. It opens at `10%` of the play-rate-adjusted montage duration and lasts `60%` of that duration, with a hard `0.75 s` ceiling; its nominal interval is therefore `[10%, 70%]`, shortened only by the ceiling. Every `Damage` notify is authored at or before the `10%` opening point so it can schedule that exact absolute window. A late notify cannot slide or reopen the interval after its close. Montage asset plus engine montage-instance ID define one logical swing. Repeated notifies, explicit stop calls and duplicate start requests for that same playback cannot reopen the window, advance its sequence or clear its target set. A newer montage instance atomically replaces the previous authority record so fast combos are not dropped, while stale/replayed sequences remain rejected and the server keeps an independent per-sequence target set. One actor can therefore receive at most one point-damage application per swing.

Authored montages whose slot is already `Emote` are played directly instead of being rebuilt from their first animation segment. This preserves the cooked sections, full timing, notifies and derived animation data used by Shipping. Dynamic slot conversion remains restricted to inputs whose authored slot differs from the requested slot. Emote root-motion suppression is owned per animation instance with one token per playback, so an interrupted montage cannot restore root motion while its replacement is still active or leave the instance in a stale suppression mode.

Only registered blocking primitive contacts produce feedback or a damage submission. Their original `FHitResult` keeps the physical material used by the authored solid/glass impact feedback. Targets with QCombat pass through the native permission and damage funnel. The shared QWeapon receiver contract also recognizes inherited Blueprint `AnyDamage` and `PointDamage` implementations, even when their class default object has no native damage delegate binding, so grates and other Blueprint destructibles receive the standard Unreal point-damage event in cooked builds. The independent `CanBeDamaged` gate remains mandatory and plain damageable actors without a real damage contract remain excluded. Trigger volumes and overlap-only helpers are not treated as melee impacts.
...
- inherited Blueprint damage-event receiver recognition without broadening eligibility to plain actors;
- statless world initialization at authored maximum life followed by an explicit persisted-stat override.

The test source has direct `QCombat` and `QWeapon` dependencies in `QAutomatedTestSuite.Build.cs` and matching plugin entries. The integrated suite currently passes `23/23`, including all six focused melee tests, statless initialization, the committed-state bridge and the authored wrapper-default contract. This proves the native state/policy/funnel and static melee invariants; it does not replace the remaining player-visible, network and packaged gates.
