...
- **État :** deux premiers chantiers P0 terminés ; troisième chantier personnage/gravité avec cleanup meshless J3 et gate Linux Shipping terminés, ownership J4/J5/J6 et requête monde typée compilés, resolver QAI supprimé et `15/15` QATS verts, validations runtime/réseau et runtime AILean encore ouvertes ; QCameraControl terminé ; QSpringArm intégré, wrapper et plugin legacy supprimés, matrice visuelle élargie encore ouverte ; SwimmingDetection natif intégré, wrapper supprimé et runtime local vert, matrice réseau/dedicated encore ouverte ; les noyaux Weapon, Inventory/QStorage et Combat sont compilés et verts en QATS mais restent hors du chemin Blueprint production tant que leurs adapters et reclassifications transactionnelles ne sont pas terminés
- **Projet :** QANGA, UE 5.7
...
| **P1 — noyau source vert, intégration ouverte** | Noyau Inventory | `InventoryComponent` : `1 591` nœuds ; `Obj_ItemInstance` : `282` ; `Lib_Inventory` : `303` ; `ItemsManagerGS` : `547`. `QInventory` possède records, codec, validation et transfert atomique (`12/12` QATS). Le backend QStorage natif conserve les générations exactes, échoue fermé sur snapshot incomplet et draine explicitement les écritures (`11/11` QATS). Aucun caller production ne l'utilise encore et le coordinateur durable neutre à deux endpoints reste absent. | Poser le premier adapter Inventory live, résoudre les endpoints côté serveur, journaliser la paire, puis retirer le remove/recreate Blueprint uniquement après restart et réseau. `InventoryComponent_C` reste une façade temporaire. |
| **P1 — noyau source vert, intégration ouverte** | Noyau Combat | `CombatComponent` : `550` nœuds ; `Lib_Combat` : `171`, dont matrice de factions à `103` nœuds. `QCombat` possède vie, permissions, faction, funnel dégâts et snapshots (`6/6` QATS), mais aucun consumer production n'est encore typé et l'asset partagé compte `525` referencers directs. | Migrer d'abord les frontières QWeapon/QAI/QPolice/QModule sans cycle, puis reclasser transactionnellement les familles Blueprint. Drops, récompenses et FX restent consommateurs post-commit. |
...
La source J5 est compilée et chargée avec son export inter-module GravityScape disponible pour QAI et QATS. L'intégration authored est appliquée aux deux bases ALS : composant personnage présent, alignement smooth uniquement sur le joueur et propriété d'alignement capsule explicitement désactivée sur AILean pour laisser QAI seul propriétaire de sa rotation. Les appels, le dispatcher, la fonction et les variables Blueprint remplacés ont été supprimés. Le probe d'un enfant avait prouvé que les deltas de CDO descendants survivent volontairement à la propagation des defaults parents ; la solution n'est donc ni un setter RzDirectMCP spécial ni douze patches enfants. L'applicateur prend maintenant possession du contrat Ninja requis au runtime et le restitue exactement. Les deux bases ALS et les douze descendants AILean recompilent à `0` erreur / `0` warning. Le probe joueur courant confirme `BaseGravityScale=1`, réplication gravité désactivée et writers concurrents coupés ; un enfant AILean en déplacement/combat reste à revalider en runtime.
...
La session PIE antérieure n'est pas une preuve runtime : le patch UCLASS Live Coding avait laissé la propriété SCS `GravityLocomotion` typée `ULIVECODING_*` à `None`, et les getters intermédiaires répétaient trois `Accessed None` par Tick. Le bridge final ne lit pas cette propriété et le batch authored a supprimé ces lecteurs. Le build froid complet relie désormais QATS, l'éditeur redémarré charge exactement les `15` tests présents dans la source et la passe `QATS.GravityScape` est verte à `15/15`. Le test locomotion couvre le premier callback, le cache de vélocité, le callback suivant et un sous-step, les entrées invalides et le teardown. Les validations runtime/réseau restent ouvertes.
...
Le core `QWeapon` possède la cadence, les transactions ammo/reload, le transform de tir et l'appel unique au sous-système balle. Son registre échantillonne désormais les providers hors itération de la map live, puis revalide identité stable et génération de topologie avant publication ; un poll identique ne publie plus un faux changement. Les suites `QWeapon.FireControl` et `QWeapon.Targeting` sont vertes à `3/3` et `7/7`. Le Blueprint firearm reste toutefois autoritaire tant que le premier adapter NashV2 n'est pas intégré et dépouillé de ses writers concurrents.

`QInventory` est vert à `12/12` avec transfert whole-record, rollback exact et relectures synchrones réentrantes explicitement bornées. `QStorage` est vert à `11/11` avec suivi exact des générations, y compris la génération zéro, refus read-only des keysets incomplets, propagation des échecs de drain, poursuite des segments sains et abandon explicite des prepares non écrits au shutdown. Ces deux cores restent dormants en production : le coordinateur neutre du journal durable à deux endpoints et le premier adapter live sont encore requis avant toute suppression Blueprint.
