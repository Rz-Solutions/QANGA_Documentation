...
- Le dernier log client a invalidé à son tour ce contrat de portée secondaire : des hovercrafts exposaient leur prompt avec des points de sortie à `375–437 cm`, un siège à environ `248 cm`, et certains vaisseaux ne résolvaient aucun point de sortie malgré un composant `Driver` valide. Le prompt local et l'autorisation serveur ne décrivaient donc pas la même interaction.
- L'exact custom target émis par `HasInteraction` est désormais l'unique ancre d'entrée, de feedback et de dispatch. Quand ce prompt est visible, aucune seconde distance, aucun angle, aucun `SlotKey` et aucun point de sortie ne peuvent invalider l'action. Le serveur conserve l'ownership et l'enregistrement du composant, l'éligibilité de possession, les valeurs finies, la visibilité contre les blockers étrangers au véhicule et l'autorisation de la cible exacte.
...
- Le diagnostic agrégé `[ServerValidationDiag]`, toujours limité par le throttle interaction à une ligne par seconde, expose le nom et la distance informative de l'ancre exacte. Les helpers réfléchis de slot/sortie, le test d'intégration associé et l'ancien rejet de portée ont été supprimés au lieu de rester comme chemin mort.
- Les unités concernées compilent par Live Coding et les quatre QATS source conservés passent. La DLL QATS chargée conserve encore l'enregistrement de l'ancien test supprimé jusqu'au prochain build Editor froid ; ce cinquième résultat historique n'est pas une preuve du source courant.
- La sortie de vaisseau et l'entrée dédiée des véhicules sont confirmées sur le précédent Development déployé. L'alignement final prompt/action sur hovercrafts et vaisseaux reste à rejouer dans un client et un serveur fraîchement reconstruits.
...
- Une vraie entrée doit toujours produire le custom target de siège authoré, qui est revalidé comme l'exacte cible visible sans second contrat de portée. À l'inverse, un `HasInteraction=true` accompagné d'un custom target strictement nul désigne une action actor-level : le serveur la repasse par la requête de vue exacte au lieu de l'interpréter comme un siège invalide. Un pointeur non nul mais invalide reste un échec explicite.
