...
## Missile-threat presentation ownership

The native targeted-by registry intentionally contains every gameplay targeter, including AI and vehicle combat actors. The legacy presentation path interpreted both individual additions and the generic changed-list payload as missile threats, so filtering only the add/remove events still allowed an AI lock to sustain the cockpit warning. The registry now captures one explicit `QWeapon.MissileThreat` classification when each targeter enters the canonical funnel. Its raw query remains complete for combat ownership, while the changed event and the dedicated missile-threat query expose only entries captured as missile threats; non-missile registry mutations no longer wake warning consumers. The cockpit warning widget uses that dedicated view for visibility and sound cadence instead of re-reading the raw list. Tag changes between insertion and removal cannot desynchronise cleanup. The production missile Blueprint authors the tag on its CDO, and the vehicle component removes the corresponding tracker on the native removal event. No class-name, component-name, or asset-path heuristic remains. Source and Blueprint compilation are green, as are the focused lifecycle, decision-math and QCombat provider-lifecycle QATS; a fresh packaged AI-lock and real-missile scenario is required for final runtime proof.

The parked-vehicle exclusion also has one central typed classification and policy, but two acquisition consumers previously bypassed it. Automatic nearest-hostile sensing now rejects an unpossessed vehicle unless it is a registered QAI traffic agent, and the state processor re-evaluates the same policy before retaining a target that may have become parked after acquisition. Possessed vehicles, authored `QMachine` actors and registered traffic combatants remain eligible. Both focused ownership/classification QATS are green; the final packaged gate must cover a parked player vehicle, an actively piloted vehicle and traffic-ship combat.
