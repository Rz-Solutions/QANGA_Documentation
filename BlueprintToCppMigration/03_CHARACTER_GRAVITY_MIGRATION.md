...
- **État :** J0/J1/J2 terminés ; cleanup meshless J3 et son gate Linux Shipping terminés ; J4/J5/J6 et la requête monde typée compilent en build Editor froid, `15/15` QATS passent ; cleanup joueur, closure Blueprint et stress PIE QPlatform à `360 FPS` revalidés ; runtime AILean, réseau et nouveau build packagé restent ouverts
...
Les logs des deux clients packagés isolent le gel signalé après une sortie de toit : aucune phase attach/detach, ignore-moving, gravité ou trace QPlatform ne dépasse la milliseconde, tandis que Slate émet des milliers d'assertions d'entrées widget invalides pendant la totalité du blocage. La divergence venait de `Slate.EnableGlobalInvalidation`, activé uniquement dans les builds packagés alors que le HUD ajoute et retire des widgets dynamiques. Le projet revient au mode d'invalidation standard dans tous les targets et les diagnostics temporaires QPlatform sont supprimés ; l'optimisation réelle qui énumère une seule fois les actors attachés reste conservée. La fermeture de ce gate exige une nouvelle build Development puis Shipping, une sortie/réentrée répétée sur les toits de véhicules et l'absence de nouvelle assertion `SlateInvalidationRoot` dans leurs logs.
