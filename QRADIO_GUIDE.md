...
**À finaliser / prévu :**
- 🔁 **Lecture continue** : **voie retenue par Benja le 2026-09-14 : un bloc d'antenne monté par station** (musique, voix et pubs mixées hors moteur), une seule piste dans `Music`, rejouée en boucle par le Wave Player musique (`Loop = true`, mesuré le 2026-09-14 sur `MS_QRadioStation` et `MS_QRadio_ChillFM`, graphes identiques à la piste près). La position live vaut `horloge serveur mod TrackMeta[0].Duration` : cette durée doit être **exactement** la `Duration` du SoundWave, sinon la radio démarre au mauvais endroit et deux auditeurs ne sont plus synchronisés (cas réel corrigé le 2026-09-14 : I-C News portait 259.68 s pour une émission de 1475.37 s). Ces MetaSounds n'ont **pas** d'entrée `Play` : inutile avec un bloc unique, l'index ne change jamais. Une playlist multi-pistes (avance d'index) reste non câblée.
