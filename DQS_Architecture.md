...
- **Attribution au joueur (fix 2026-09-02)** : `FinishObjective()` / `FailObjective()` resolvent le couple (quete, joueur) a crediter dans cet ordre : (1) `OwningQuestID` + `AssignedPlayerID` de l'instance si cette quete est `InProgress` pour ce joueur et liste encore l'objectif ; (2) scan d'identite (la quete dont `ObjectiveInstances[ObjectiveID]` EST cet objet) ; (3) repli historique = premier joueur de `PlayerQuestStates` dont une quete en cours contient l'objectif (helper `ResolveOwningRuntimeQuest`, `QuestObjectiveBase.cpp`). Avant ce fix, seul (3) existait : sur serveur dedie, l'achat des 9 mm de Q008 par le second joueur connecte completait `O_Shop_Munition` chez le PREMIER joueur (mesure 2026-09-02). `IncrementProgress()` utilisait deja `AssignedPlayerID` et n'a jamais eu ce defaut. Le repli (3) reste ambigu a plusieurs joueurs et journalise un `DQS_LOG_WARNING` dans ce cas.
...
15. **Miroirs `Serialized*` : regeneres UNIQUEMENT quand vides** (decouvert 2026-07-16 via les outils quete CLIScape). L'`operator<<` de `FQuestDefinition` (`QuestDataTypes.h:1642/1656/1701`) ne reconstruit `SerializedObjectives`/`SerializedActions`/`SerializedConditions` depuis les objets vivants QUE si le tableau correspondant est vide. Le cache editeur (`QuestDataMap`) garde des miroirs perimes a vie (la regeneration s'opere sur une copie locale dans le chemin de sauvegarde). La fenetre Quest Editor contourne cela en VIDANT elle-meme les trois tableaux avant sauvegarde (`QuestEditorWindow.cpp:4071/4340/4687` dans `UpdateCurrentQuest`). Tout code qui appelle `UpdateQuest` directement (scripts, outils CLIScape) DOIT faire ce flush lui-meme, sinon le JSON re-ecrit les composants d'epoque et ecrase les modifications vivantes. Les outils CLIScape le font via `QuestToolHelpers::RefreshSerializedMirrors`. Consequence duale : dupliquer depuis le cache reconstruit une copie VIDE ; il faut repartir du fichier (`LoadQuestFromFile`).

16. **La collecte des conditions du serializer omet `SkipConditions`** : la passe de sauvegarde (`QuestDataTypes.h:1713-1732`) recolte AcceptanceConditions + ActivationConditions + ProgressCondition + FailureCondition des objectifs + ExecutionCondition des actions, mais PAS `Objective->SkipConditions`. Une skip condition presente uniquement dans le slot vivant de l'objectif est silencieusement perdue au save si `SerializedConditions` est regenere par cette passe. L'UI n'y est pas exposee (ses conditions vivent dans `AcceptanceConditions` avec `ConditionType`+`ParentComponentID`, et elle reconstruit `SerializedConditions` elle-meme). Piege associe : `UQuestComponentManager::CreateCondition` ne recopie PAS le nom d'objet dans `ConditionID` (contrairement a `CreateObjective`/`CreateAction`) ; il faut assigner `ConditionID` explicitement apres creation.

17. **Evenements globaux sans PlayerID (CORRIGE 2026-09-02, seconde passe)** : `OnQuestStateChangedEvent`, `OnQuestUpdatedEvent`, `OnQuestCompletedEvent`, `OnQuestFailedEvent`, `OnQuestAbandonedEvent` sont diffuses avec le seul QuestID et CHAQUE `UPlayerQuestComponent` du serveur y est abonne (`SetupEventListeners`). Avant le fix, l'abandon ou la completion chez un joueur retirait la quete des tableaux repliques de l'autre joueur partageant la quete, lui jouait le son de completion et desinscrivait ses objectifs de position. Mecanisme retenu (signatures `BlueprintAssignable` INCHANGEES) : le subsystem porte un contexte transient `CurrentEventPlayerID` (`GetCurrentEventPlayerID()`), pose par `TGuardValue` dans les variantes `Broadcast*EventForPlayer(..., PlayerID)` utilisees par `AssignQuestToPlayer`, `AbandonQuest`, `CompleteQuest`, `FailQuest`, `UpdateQuestObjectiveProgress` (y compris le delegue natif `OnQuestObjectiveCompleted`), `PreparePickupObjectiveForActivation`, `BroadcastQuestObjectiveActivatedEvent`, `BroadcastQuestTrackingChangedEvent` et `AQuestInstanceActor`. Cote ecoute, `UPlayerQuestComponent::ShouldHandleGlobalQuestEvent()` ignore tout evenement dont le joueur est renseigne et n'est pas le sien (un contexte `NAME_None` = diffuseur legacy, comportement historique conserve), et `ULocationObjective::OnGuardianObjectiveCompleted` applique le meme filtre sur `AssignedPlayerID`. Les anciens helpers `Broadcast*Event` (UFUNCTION) restent en place et sans contexte. Restes hors perimetre : `CleanupQuestRegistryEntries(QuestID)` reste global par quete (les cles du registre sont `<ActionID>_<Cle>`, sans joueur ni quete : le prefixe `Quest_<ID>_` ne les touche pas, c'est un design de registre partage, pas un bug de fuite), et `BroadcastQuestUpdatedEvent(QuestID, "ObjectiveCleanup")` (nettoyage de references, pas par joueur). Piege inchange : `OnQuestObjectiveCompletedEvent` (dynamique) n'est JAMAIS diffuse (`BroadcastQuestObjectiveCompletedEvent` sans appelant), le handler `OnGlobalQuestObjectiveCompleted` est du code mort mais porte desormais le filtre par joueur. Bonus de la meme passe : `CompleteQuest`/`FailQuest` resolvent le controleur du joueur par `FindPlayerControllerForPlayerID` au lieu de `GetFirstPlayerController`, et `Server_SetAuthoritativePlayerID` n'initialise plus que le LocalPlayer du controleur proprietaire (serveur d'ecoute).
